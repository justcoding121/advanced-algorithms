...
{
    public class BinomialTreeNode<T> : IComparable where T : IComparable
    {
        internal T Value { get; set; }
        internal int Degree => Children.Count;

        internal BinomialTreeNode<T> Parent { get; set; }
        internal List<BinomialTreeNode<T>> Children { get; set; }
        ...
        {
            return this.Value.CompareTo((obj as BinomialTreeNode<T>).Value);
        }
    }
    ...
    public class AsBinomialMinHeap<T> where T : IComparable
    {
        ...
        internal AsDoublyLinkedList<BinomialTreeNode<T>> heapForest
            = new AsDoublyLinkedList<BinomialTreeNode<T>>();

        /// <summary>
        /// O(log(n)) complexity
        /// </summary>
        /// <param name="newItem"></param>
        public BinomialTreeNode<T> Insert(T newItem)
        {
            var newNode = new BinomialTreeNode<T>(newItem);

            var newHeapForest = new AsDoublyLinkedList<BinomialTreeNode<T>>();
        ...
        public void DecrementKey(BinomialTreeNode<T> node)
        {
            var current = node;
        ...
        private void MergeSortedForests(AsDoublyLinkedList<BinomialTreeNode<T>> newHeapForest)
        {
            ...
            while (@new != null && current != null)
            {
                ...
                {
                    heapForest.InsertBefore(current, new DoublyLinkedListNode<BinomialTreeNode<T>>(@new.Data));
                    @new = @new.Next;
                }
                else
                {
                    //equal
                    heapForest.InsertAfter(current, new DoublyLinkedListNode<BinomialTreeNode<T>>(@new.Data));
                    current = current.Next;
                    @new = @new.Next;
                }
            ...
            while (@new != null)
            {
                heapForest.InsertAfter(heapForest.Tail, new DoublyLinkedListNode<BinomialTreeNode<T>>(@new.Data));
                @new = @new.Next;
            }
