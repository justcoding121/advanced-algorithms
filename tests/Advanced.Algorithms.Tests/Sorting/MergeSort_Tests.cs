...
namespace Advanced.Algorithms.Tests.Sorting
{
    ...
    public class MergeSortTests
    {
        ...
        [TestMethod]
        public void MergeSort_Ascending_Smoke_Test()
        {
        ...
        [TestMethod]
        public void MergeSort_Ascending_Stress_Test()
        {
            var rnd = new Random();
            var nodeCount = 1000;
            ...
            var result = MergeSort<int>.Sort(randomNumbers.ToArray());

            for (var i = 1; i <= nodeCount; i++) Assert.AreEqual(i, result[i - 1]);
        }

        [TestMethod]
        public void MergeSort_Descending_Stress_Test()
        {
            var rnd = new Random();
            var nodeCount = 1000;
            var randomNumbers = Enumerable.Range(1, nodeCount)
        ...
        [TestMethod]
        public void MergeSort_Corner_Cases()
        {
            CollectionAssert.AreEqual(new int[0], MergeSort<int>.Sort(new int[0]));
            CollectionAssert.AreEqual(new[] { 7 }, MergeSort<int>.Sort(new[] { 7 }));
            CollectionAssert.AreEqual(new[] { 1, 2, 2, 3 }, MergeSort<int>.Sort(new[] { 3, 1, 2, 2 }));
            CollectionAssert.AreEqual(new[] { 1, 2, 3, 4 }, MergeSort<int>.Sort(new[] { 1, 2, 3, 4 }));
            CollectionAssert.AreEqual(new[] { 1, 2, 3, 4 }, MergeSort<int>.Sort(new[] { 4, 3, 2, 1 }));
        }

        [TestMethod]
        public void MergeSort_Oracle_Against_Array_Sort()
        {
        ...
        private static void AssertMatchesArraySort(int[] input)
        {
            ...
            CollectionAssert.AreEqual(expectedAsc, MergeSort<int>.Sort((int[])input.Clone()));

            var expectedDesc = (int[])expectedAsc.Clone();
            Array.Reverse(expectedDesc);
            CollectionAssert.AreEqual(expectedDesc, MergeSort<int>.Sort((int[])input.Clone(), SortDirection.Descending));
        }
    }
}
