...
namespace Advanced.Algorithms.Tests.Sorting
{
    ...
    public class HeapSortTests
    {
        ...
        [TestMethod]
        public void HeapSort_Oracle_Against_Array_Sort()
        {
        ...
        private static void AssertMatchesArraySort(int[] input)
        {
            ...
            CollectionAssert.AreEqual(expectedAsc, HeapSort<int>.Sort((int[])input.Clone()));

            var expectedDesc = (int[])expectedAsc.Clone();
            Array.Reverse(expectedDesc);
            CollectionAssert.AreEqual(expectedDesc, HeapSort<int>.Sort((int[])input.Clone(), SortDirection.Descending));
        }
    }
}
