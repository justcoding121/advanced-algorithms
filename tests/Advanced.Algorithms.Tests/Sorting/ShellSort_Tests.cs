...
namespace Advanced.Algorithms.Tests.Sorting
{
    ...
    public class ShellSortTests
    {
        ...
        [TestMethod]
        public void ShellSort_Corner_Cases()
        {
            CollectionAssert.AreEqual(new int[0], ShellSort<int>.Sort(new int[0]));
            CollectionAssert.AreEqual(new[] { 7 }, ShellSort<int>.Sort(new[] { 7 }));
            CollectionAssert.AreEqual(new[] { 1, 2, 2, 3 }, ShellSort<int>.Sort(new[] { 3, 1, 2, 2 }));
            CollectionAssert.AreEqual(new[] { 1, 2, 3, 4 }, ShellSort<int>.Sort(new[] { 1, 2, 3, 4 }));
            CollectionAssert.AreEqual(new[] { 1, 2, 3, 4 }, ShellSort<int>.Sort(new[] { 4, 3, 2, 1 }));
        }

        [TestMethod]
        public void ShellSort_Oracle_Against_Array_Sort()
        {
        ...
        private static void AssertMatchesArraySort(int[] input)
        {
            ...
            CollectionAssert.AreEqual(expectedAsc, ShellSort<int>.Sort((int[])input.Clone()));

            var expectedDesc = (int[])expectedAsc.Clone();
            Array.Reverse(expectedDesc);
            CollectionAssert.AreEqual(expectedDesc, ShellSort<int>.Sort((int[])input.Clone(), SortDirection.Descending));
        }
    }
}
