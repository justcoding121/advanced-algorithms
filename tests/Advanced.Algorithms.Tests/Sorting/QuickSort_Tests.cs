...
namespace Advanced.Algorithms.Tests.Sorting
{
    ...
    public class QuickSortTests
    {
        ...
        [TestMethod]
        public void QuickSort_Corner_Cases()
        {
            CollectionAssert.AreEqual(new int[0], QuickSort<int>.Sort(new int[0]));
            CollectionAssert.AreEqual(new[] { 7 }, QuickSort<int>.Sort(new[] { 7 }));
            CollectionAssert.AreEqual(new[] { 1, 2, 2, 3 }, QuickSort<int>.Sort(new[] { 3, 1, 2, 2 }));
            CollectionAssert.AreEqual(new[] { 1, 2, 3, 4 }, QuickSort<int>.Sort(new[] { 1, 2, 3, 4 }));
            CollectionAssert.AreEqual(new[] { 1, 2, 3, 4 }, QuickSort<int>.Sort(new[] { 4, 3, 2, 1 }));
        }

        [TestMethod]
        public void QuickSort_Oracle_Against_Array_Sort()
        {
        ...
        private static void AssertMatchesArraySort(int[] input)
        {
            ...
            CollectionAssert.AreEqual(expectedAsc, QuickSort<int>.Sort((int[])input.Clone()));

            var expectedDesc = (int[])expectedAsc.Clone();
            Array.Reverse(expectedDesc);
            CollectionAssert.AreEqual(expectedDesc, QuickSort<int>.Sort((int[])input.Clone(), SortDirection.Descending));
        }
    }
}
