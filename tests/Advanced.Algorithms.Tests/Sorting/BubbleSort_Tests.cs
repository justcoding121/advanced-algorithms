...
namespace Advanced.Algorithms.Tests.Sorting
{
    ...
    public class BubbleSortTests
    {
        ...
        {
            var result = BubbleSort<int>.Sort(TestArray, SortDirection.Descending);
        ...
        public void BubbleSort_Descending_Stress_Test()
        {
            ...
            var result = BubbleSort<int>.Sort(randomNumbers.ToArray(), SortDirection.Descending);

            for (var i = 0; i < nodeCount; i++) Assert.AreEqual(randomNumbers.Count - i, result[i]);
        }

        [TestMethod]
        public void BubbleSort_Corner_Cases()
        {
            CollectionAssert.AreEqual(new int[0], BubbleSort<int>.Sort(new int[0]));
            CollectionAssert.AreEqual(new[] { 7 }, BubbleSort<int>.Sort(new[] { 7 }));
            CollectionAssert.AreEqual(new[] { 1, 2, 2, 3 }, BubbleSort<int>.Sort(new[] { 3, 1, 2, 2 }));
            CollectionAssert.AreEqual(new[] { 1, 2, 3, 4 }, BubbleSort<int>.Sort(new[] { 1, 2, 3, 4 }));
            CollectionAssert.AreEqual(new[] { 1, 2, 3, 4 }, BubbleSort<int>.Sort(new[] { 4, 3, 2, 1 }));
        }

        [TestMethod]
        public void BubbleSort_Oracle_Against_Array_Sort()
        {
        ...
        private static void AssertMatchesArraySort(int[] input)
        {
            ...
            CollectionAssert.AreEqual(expectedAsc, BubbleSort<int>.Sort((int[])input.Clone()));

            var expectedDesc = (int[])expectedAsc.Clone();
            Array.Reverse(expectedDesc);
            CollectionAssert.AreEqual(expectedDesc, BubbleSort<int>.Sort((int[])input.Clone(), SortDirection.Descending));
        }
    }
}
